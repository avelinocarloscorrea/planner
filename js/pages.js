...
PAGE_DRAW.tab = (pen, box, o, ctx) => {
  ...
  const size = pen.fitText(t, W - 24, Math.min(30, W * 0.14), 12, true, ctx.hfam);
  pen.text(t, cxT, cy, { size, font: 'bold', color: ctx.ink, align: 'c', baseline: 'middle', tracking: 1.5, family: ctx.hfam });
};
...
PAGE_DRAW.quote = (pen, box, o, ctx) => {
  ...
  const size = pen.fitText(txt.length > 60 ? 'x'.repeat(30) : txt, box.w * 0.86, 17, 10, false, ctx.hfam);
  const lines = pen.wrapText(txt, box.w * 0.86, size, false, 8, ctx.hfam);
  const lh = size * 1.5 / PT;
  let ty = midY - (lines.length - 1) * lh / 2;
  lines.forEach(l => { pen.text(l, cx, ty, { size, font: 'it', color: ctx.ink, align: 'c', baseline: 'middle', family: ctx.hfam }); ty += lh; });
  if (o.author) {
    pen.line(cx - 14, ty + 3, cx + 14, ty + 3, { w: 0.4, color: ctx.accent });
    pen.text('— ' + String(o.author), cx, ty + 9, { size: 9, color: ctx.faint, align: 'c', baseline: 'top' });
  }
};
